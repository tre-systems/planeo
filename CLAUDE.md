...
- Work on `main` and push there. A push to `main` runs CI and auto-deploys to
  Fly.io once `FLY_API_TOKEN` is set (the app is not currently deployed). After a
  code change confirm CI is green and, once deployed, smoke-test the live app.
  Docs-only changes just need commit + push.
